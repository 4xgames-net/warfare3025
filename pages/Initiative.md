...
- ### Overview
	- Instead of a turn by turn resolution where one unit activates after another, units gain action-ticks when they perform actions.
...
- ### Interrupts
	- possible if interrupting unit sees the action and has less ticks than the actor after finishing the action
		- #+BEGIN_EXAMPLE
	...
	- If the interrupting action is specifically designed to prevent the action question, it may do so. Ranged attack cannot be dodged.
		- #+BEGIN_EXAMPLE
		  ...
		  #+END_EXAMPLE
	- Interrupting actions not specifically countering the original action only occur first if the ticks of the interrupting unit are still lower after the resolution of the entire situation.
		- #+BEGIN_EXAMPLE
